	...
	for root, _, fnames in sorted(os.walk(dir)):
		for fname in fnames:
			if is_image_file(fname):
				...
				path_names = path_names.replace('fashion_dataimg_highres', 'fashion')
				img = Image.open(path)
				img_resize = img.resize((176, 256))
				if path_names in train_images:
					img_resize.save(os.path.join(train_root, path_names))
					print('train %s' % os.path.join(train_root, path_names))
				elif path_names in test_images:
					img_resize.save(os.path.join(test_root, path_names))
					print('test %s' % os.path.join(test_root, path_names))

make_dataset('fashion_data/img_highres')