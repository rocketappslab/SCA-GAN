...
class TransferModel(BaseModel):
    ...
    def forward(self):
        self.fake_p2 = self.netG(self.input_EP1, self.input_BP2, self.input_BEP2)
    ...
    def backward_G(self):
        ...
        if self.opt.with_D_PP:
            pred_fake_PP = self.netD_PP(torch.cat((self.fake_p2, self.input_EP1), 1))
        ...
        if self.opt.L1_type == 'l1_plus_perL1' :
            losses = self.criterionL1(self.fake_p2, self.input_EP2)
            self.loss_G_L1 = losses[0]
            self.loss_originL1 = losses[1].data
            self.loss_perceptual = losses[2].data
        else:
            self.loss_G_L1 = self.criterionL1(self.fake_p2, self.input_EP2) * self.opt.lambda_A
    ...
    def backward_D_PB(self):
        real_PB = torch.cat((self.input_EP2, self.input_BP2), 1)
    ...
    def backward_D_PP(self):
        real_PP = torch.cat((self.input_EP2, self.input_EP1), 1)
        # fake_PP = self.fake_PP_pool.query(torch.cat((self.fake_p2, self.input_P1), 1))
        fake_PP = self.fake_PP_pool.query( torch.cat((self.fake_p2, self.input_EP1), 1).data )
    ...
    def get_current_visuals(self):
        height, width = self.input_EP1.size(2), self.input_EP1.size(3)
